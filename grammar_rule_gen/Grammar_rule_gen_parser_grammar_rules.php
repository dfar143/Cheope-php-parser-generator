...
class Parser_grammar_rule_produz_1 extends Parser_grammar_rule
{
 ...
 {	
 	...
 	{
 	 $res12 = $parser->match(Token::TYPE_SPECIAL_ITEM,TOKEN_VAL_EQUAL);
 	 if(! $res12)
 	 {
 	 	$this->backtrack($localTokensBufferPointer);
 	 }
 ...
 function space()
 {
  ...
  $res1 = $parser->match(Token::TYPE_DELIM,TOKEN_VAL_WS);
  if(! $res1)
   $this->backtrack($localTokensBufferPointer);
 ...
 function produzLeft()
 {
  ...
  else
  {
   ...
   $res12 = $parser->match(Token::TYPE_LEXICAL_ELEMENT,TOKEN_VAL_NONTERMINALE);
   if(! $res12)
   {
    $this->backtrack($localTokensBufferPointer);
   }
 ...
 function produzRight1()
 {
  ...
  $tok = &$parser->getCurrentToken();
  $res11 = $parser->match(Token::TYPE_LEXICAL_ELEMENT,TOKEN_VAL_TERMINALE); 
  if(! $res11)
  {
   $this->backtrack($localTokensBufferPointer);
  }
  ...
  {
   $tok = &$parser->getCurrentToken();
   $res21 = $parser->match(Token::TYPE_LEXICAL_ELEMENT,TOKEN_VAL_NONTERMINALE);
   if(! $res21)
   {
   	$this->backtrack($localTokensBufferPointer);
   }
 ...
 function produzRight2()
 {
  ...
  if(! $res1)
  {
   $res21 = $parser->match(Token::TYPE_SPECIAL_ITEM,TOKEN_VAL_OR);
   if(! $res21)
   {
    $this->backtrack($localTokensBufferPointer);
   }
