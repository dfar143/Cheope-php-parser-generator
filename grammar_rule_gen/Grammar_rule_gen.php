...
class Grammar_rule_gen extends Generic_interface
{
 const VAR_RES="res";
...
private function getCodeFromTransientLexema($actComposedRightElement)
 { 
 	...
 	{
 	 $body .= '$parser->match(' . "Token::" . Token::TYPE_SUFFIX . 
 	 STRING_UNDERSCORE . strtoupper($tok3->getType()) . 
 	 STRING_COMMA . strtoupper($prjName) . STRING_UNDERSCORE . "TOKEN" . 
 	 STRING_UNDERSCORE . Token::VAL_SUFFIX . STRING_UNDERSCORE . strtoupper($tok3->getVal()) . 
 	 ')';
 	}
 	else
 	{
...
// 
 function exec($actCt=0)
 {
   ...
   $methodBody .= '$res1 = $parser->match(Token::TYPE_DELIM,' . strtoupper($prjName) . STRING_UNDERSCORE . "TOKEN" . 
 	 STRING_UNDERSCORE . Token::VAL_SUFFIX . STRING_UNDERSCORE . 'WS);' . $phpClassGen->EOL();
