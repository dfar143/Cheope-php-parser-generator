...
$rule1 = new Lex_rule(LEX_RULE_1);
$rule2 = new Lex_rule(LEX_RULE_2);
$rule3 = new Lex_rule(LEX_RULE_3);
$rule4 = new Lex_rule(LEX_RULE_4);
$rule5 = new Lex_rule(LEX_RULE_5);
//$dfa6 = &new Lex_dfa(LEX_RULE_6);

$rule1->setRegexp("/^[A-Z][A-Z_a-z0-9]*/");
$rule1->setTokenType(Token::TYPE_LEXICAL_ELEMENT);
$rule1->setTokenVal(TOKEN_VAL_NONTERMINALE);

$rule2->setRegexp("/^[a-z][A-Z_a-z0-9]*/");
$rule2->setTokenType(Token::TYPE_LEXICAL_ELEMENT);
$rule2->setTokenVal(TOKEN_VAL_TERMINALE);

$rule3->setRegexp("/^[\s]+/");
$rule3->setTokenType(Token::TYPE_DELIM);
...
$rules = new Lex_rules_container(LEX_RULE_CONTAINER_1);
$rules->add($rule1);
$rules->add($rule2);
$rules->add($rule3);
$rules->add($rule4);
...
$grRules = new Parser_grammar_rules_container(PARSER_GRAMMAR_RULE_CONTAINER_1);
$grRules->add($grRule1);
?>